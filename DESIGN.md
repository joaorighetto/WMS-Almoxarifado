...
- `.table-sticky-header`: cabeçalho fixo quando a listagem for longa. Aplicado ao wrapper, dá a ele um scrollport vertical próprio (altura máxima de 60% da viewport) — sem isso `position: sticky` não tem contra o que colar. Wrapper sem essa classe (ex.: tabela vazia) rola com a página.
...
- Quando uma coluna for ordenável conforme os requisitos da superfície, seu cabeçalho deve oferecer a ação de ordenação e indicar claramente a direção/estado atual (nenhuma ordenação implementada ainda).
...
- **Só Anterior / Próxima + indicador da página atual** — sem lista de todos os números, para continuar utilizável com milhares de itens. À esquerda, um resumo em metadado ("Página X de Y — N no total"); à direita, os três controles com borda fina, raio `sm`, mínimo de 32px (44px sob `pointer: coarse`). A página atual não é clicável e se marca por peso semibold + borda e texto em `primary`; o controle indisponível fica em `disabled`, sem ação.
