...
| FOR | Importação do cadastro de fornecedores do SCPI | Do CSV do SCPI à consulta de fornecedores e conferência auditável da carga, para servirem de emitente nas entradas | Carga por CSV com prévia e confirmação; consulta; reimportação; resultado auditável | Cadastro manual de fornecedores; compras/licitações; contratos; dados financeiros ou fiscais além da identificação; integração automática | O: 002; R: antes de ENT | Em implementação — [spec 004](specs/004-importacao-fornecedores/spec.md) implementada no branch `004-importacao-fornecedores`, validada com o CSV real, aguardando merge em `main`; incluída por decisão do dono do produto em 2026-09-25 |
...
| FOR | `PERM-SUPPLIER-IMPORT-EXECUTE`, `PERM-SUPPLIER-IMPORT-HISTORY-VIEW`, `PERM-SUPPLIER-VIEW`; `INV-SUPPLIER-001` a `INV-SUPPLIER-005` |
...
| FOR | **Decididos na [spec 004](specs/004-importacao-fornecedores/spec.md):** recorte do CSV real de fornecedores do SCPI (disponível só localmente em `docs/CSVs/`, ignorado pelo Git: 10.035 registros, 132 colunas, `CODIF` único) — só as 8 colunas de identificação e bloqueio (FR-012), sem conta bancária, PIS, endereço nem contato; efeito da reimportação sobre fornecedor ausente do arquivo — permanece inalterado e é só contado (FR-022). Origem só por importação, permissões, dados mínimos e bloqueio decididos em 2026-09-25 e registrados nas matrizes canônicas (`PERM-SUPPLIER-*`, `INV-SUPPLIER-*`). |
