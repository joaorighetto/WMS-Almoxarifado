...
| FOR | Importação do cadastro de fornecedores do SCPI | Do CSV do SCPI à consulta de fornecedores e conferência auditável da carga, para servirem de emitente nas entradas | Carga por CSV com prévia e confirmação; consulta; reimportação; resultado auditável | Cadastro manual de fornecedores; compras/licitações; contratos; dados financeiros ou fiscais além da identificação; integração automática | O: 002; R: antes de ENT | Em especificação — [spec 004](specs/004-importacao-fornecedores/spec.md), com plano e tarefas; incluída por decisão do dono do produto em 2026-09-25 |
